...
struct MemoryArena {
    ...
    MemoryArena(std::byte* addr, size_t size) noexcept : blk_addr{addr}, blk_size{size} {}

    MemoryArena(std::span<std::byte> blk) noexcept : MemoryArena{blk.data(), blk.size()} {}
...
public:
    explicit MemoryArenaScope(MemoryArena& arena) noexcept : _restore_offset{arena.blk_offset}, _arena{&arena} {}

    ~MemoryArenaScope() noexcept { _arena->blk_offset = _restore_offset; }

    MemoryArenaScope(const MemoryArenaScope&) = delete;
    MemoryArenaScope& operator=(const MemoryArenaScope&) = delete;

    MemoryArena* arena() noexcept { return _arena; }

private:
    ptrdiff_t _restore_offset{};
    MemoryArena* _arena{};
};

template <typename T> class SimpleArenaAllocator {
public:
    using value_type = T;

    SimpleArenaAllocator(MemoryArenaScope& arena_scope) noexcept : SimpleArenaAllocator{arena_scope.arena()} {}
    SimpleArenaAllocator(MemoryArena* mem_arena) noexcept : _mem_arena{mem_arena} {}
